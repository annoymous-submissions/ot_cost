...
lams = [(2, 1), (1, 1), (1, 2), (3, 1), (1, 3), (4,1), (1,4), (5,1), (1,5)]
...
def grapher(dataset, df, save = True):
    plt.clf()
    order = [(2, 1), (1, 1), (1, 2), (1, 3), (1, 4), (1,5), (3,1), (4,1), (5,1)]
    index_1, index_2 = zip(*df.index)
    df.index = pd.MultiIndex.from_tuples(list(zip(index_1, index_2)), names=["index_1", "index_2"])
    df = df.loc[order]
    baseline = df.loc[(2,1)]
    for i in df.index:
        if i != (2,1):
            sns.lineplot(x=baseline, y=df.loc[i], alpha=0.8, marker='o', label=f'{i[0]}:{i[1]}')
    ...
    plt.xlabel(r'Cost at $\lambda=2:1$', fontsize = 12)
    plt.ylabel(f'Calculated cost', fontsize = 12)
    plt.legend(bbox_to_anchor=(1, 1), title=r"$\lambda$")
    plt.xticks()
    plt.yticks()
    if save == True:
        df.to_csv(f'{ROOT_DIR}/results/lambda_param/{dataset}_lambda_param.csv')
        plt.savefig(f'{ROOT_DIR}/results/lambda_param/{dataset}_lambda_param.pdf', bbox_inches='tight')
    plt.show()

def compute_synthetic_cost(args):
    dataset, cost, lam = args
...
def synthetic():
    ...
    all_args = [(dataset, cost, lam) for cost in costs for lam in lams]
    results_synthetic = {cost: {lam: 0} for cost in costs for lam in lams}
    with concurrent.futures.ProcessPoolExecutor(max_workers=CPU) as executor:
        for args, c in zip(all_args, executor.map(compute_synthetic_cost, all_args)):
            _, cost, lam = args
            results_synthetic[cost][lam] = c
...
def weather():
    ...
    all_args = [(dataset, cost, climate, lam) for cost, climate in zip(costs,climates) for lam in lams]
    results_weather = {cost: {lam: 0} for cost in costs for lam in lams}
    with concurrent.futures.ProcessPoolExecutor(max_workers=CPU) as executor:
        for args, c in zip(all_args, executor.map(compute_weather_cost, all_args)):
            _, cost, climate, lam = args
            results_weather[cost][lam] = c
...
def emnist():
    ...
    all_args = [(dataset, cost, lam) for cost in costs for lam in lams]
    results_emnist = {cost: {lam: 0} for cost in costs for lam in lams}
    with concurrent.futures.ProcessPoolExecutor(max_workers=CPU) as executor:
        for args, c in zip(all_args, executor.map(compute_emnist_cost, all_args)):
            _, cost, lam = args
            results_emnist[cost][lam] = c
    df = pd.DataFrame(results_emnist)
    grapher(dataset, df, save=True)

def compute_cifar_cost(args):
    dataset, _, lam, x1, y1, x2, y2 = args
    OTCost_label = ot.OTCost(dataset, {'1': x1, '2': x2}, {'1': y1, '2': y2}, lam=lam)
    return OTCost_label.calculate_ot_cost()
...
def cifar():
    ...
    results_cifar = {cost: {lam: 0} for cost in costs for lam in lams}
    all_args = []
    n_emb = 1000
    num_samples = 500
...
def ixitiny():
    ...
    results_ixitiny = {cost: {lam: 0} for cost in costs for lam in lams}
    all_args = []

    for ind, cost in zip(indices, costs):
        X1, X2 = site_embeddings[ind[0]][:], site_embeddings[ind[1]][:]
        for lam in lams:
            all_args.append((dataset, cost, lam, X1, X2))

    with concurrent.futures.ProcessPoolExecutor(max_workers=CPU) as executor:
        for args, c in zip(all_args, executor.map(compute_ixitiny_cost, all_args)):
            _, cost, lam, _, _ = args
            results_ixitiny[cost][lam] = c

    df = pd.DataFrame(results_ixitiny)
    grapher(dataset, df, save=True)


def create_dictionaries_isic(site_embeddings, site_labels, sites, NUM_SAMPLES = 900):
...
def compute_isic_cost(args):
    ind, cost, lam, DATASET, site_embeddings, site_labels = args
    importlib.reload(ot)
    if ind[0] != ind[1]:
        data, labels = create_dictionaries_isic(site_embeddings, site_labels, ind)
        ISIC_OTCost_label = ot.OTCost(DATASET, data, labels, lam=lam)
        ...
        data, labels = remove_rare_labels(data, labels, min_count=30)
        OTCost_label = ot.OTCost(DATASET, data, labels, lam=lam)
        return OTCost_label.calculate_ot_cost()
...
def isic():
    ...
    results_isic = {cost: {lam: 0} for cost in costs for lam in lams}
    all_args = [(ind, cost, lam, DATASET, site_embeddings, site_labels) for ind,cost in zip(site_pairs,costs) for lam in lams]

    with concurrent.futures.ProcessPoolExecutor(max_workers=CPU) as executor:
        for args, c in zip(all_args, executor.map(compute_isic_cost, all_args)):
            _, cost, lam, _, _, _ = args
            results_isic[cost][lam] = c
    dataset = 'ISIC'
